...
def update_operator(type, magnitude=None):

    # ox, oy, oz, oa
    operator = [0.0, 0.0, 0.0, 0.0]
    random_num = random.choice([-1, 1])

    if type == 'horizontal_shift':
        operator[0] = random.uniform(0.05, 0.45) * random_num if magnitude == None else magnitude

    elif type == 'vertical_shift':
        operator[1] = random.uniform(0.05, 0.45) * random_num if magnitude == None else magnitude

    elif type == 'zoom':
        if random_num == -1:
            operator[2] = random.uniform(0.048, 0.310) * random_num if magnitude == None else magnitude
        elif random_num == 1:
            operator[2] = random.uniform(0.053, 0.818) * random_num if magnitude == None else magnitude

    elif type == 'rotate':
        operator[3] = random.uniform(math.pi/36, math.pi/4) * random_num if magnitude == None else magnitude
...
def get_rotated_image(image, bounding_box, allow_zero_pixel, radian=None):
    ...
    operator = update_operator('rotate', magnitude=radian)
    
    rotated_image = rotate_image(image, bounding_box, allow_zero_pixel, operator)
    if rotated_image == None:
        return None
    return rotated_image, operator


def get_perturbed_image(image, bounding_box, allow_zero_pixel, type, magnitude=None):

    if type == 'rotate':
        return get_rotated_image(image, bounding_box, allow_zero_pixel, radian=magnitude)
    
    norm_box = get_normalized_bounding_box(bounding_box, image.size)
    
    operator = update_operator(type, magnitude)
    ...
    return perturbed_crop_image, operator
...
if __name__ == '__main__':
    ...
    rotated_image, operator = get_perturbed_image(image, bounding_box, allow_zero_pixel, type='horizontal_shift')
    rotated_image.show()

    shifted_image, operator = get_perturbed_image(image, bounding_box, allow_zero_pixel, type='vertical_shift')
    shifted_image.show()

    zoomed_out_image, operator = get_perturbed_image(image, bounding_box, allow_zero_pixel, type='zoom')
    zoomed_out_image.show()

    cropped_image, operator = get_perturbed_image(image, bounding_box, allow_zero_pixel, type='rotate')
    cropped_image.show()
